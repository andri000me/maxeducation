...
class Profile extends CI_Controller {
	...
	{
		$data = [
			'title' => 'MAX Education | Halaman Profile Siswa',
			'content' => $this->load->view('siswa/content_profile_view', [
				'user' => $this->Siswa_model->get_siswa_where($this->session->userdata('siswa_id')),
	...
	public function update_informasi($id_siswa)
	{
		$this->_validate_informasi();

		$data = [
			'nama_lengkap_siswa' => $this->input->post('nama_lengkap_siswa'),
		...
		if(!empty($_FILES['avatar']['name'])){
			...
			$avatar = $this->Siswa_model->get_siswa_where($this->session->userdata('siswa_id'));
		...
		if ($this->Siswa_model->update_siswa_informasi($id_siswa, $data)) {
			$response = ['status' => TRUE, 'message' => 'Data Informasi Berhasil Diubah'];
	...
	public function update_password($id_user)
	{
		...
		if ($this->User_model->update_admin_password($id_user, $data)) {
			$response = ['status' => TRUE, 'message' => 'Data Password Berhasil Diubah'];
			$this->output->set_content_type('application/json')->set_output(json_encode($response));
		}
	}
	...
	private function _validate_informasi()
	{
		...
		if($this->input->post('nama_lengkap_siswa') == '')
		{
			$data['inputerror'][] = 'nama_lengkap_siswa';
