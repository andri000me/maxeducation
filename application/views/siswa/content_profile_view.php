        ...
        <div class="box box-primary">
          <div class="box-body box-profile">
            <?php if (empty($user->avatar)): ?>
              <img src="<?= base_url('uploads/images/avatars/avatar.jpg') ?>" class="img-thumbnail img-responsive" alt="User Image" style="width: 100%;">

            <?php else: ?>
             <img src="<?= base_url('uploads/images/avatars/'.$user->avatar) ?>" class="img-thumbnail img-responsive" style="width: 100%;" id='img-upload'>
           <?php endif ?>
           

           <h3 class="profile-username text-center"><?= $user->nama_lengkap_siswa ?></h3>
          ...
          <div class="active tab-pane" id="settings">
            <form class="form-horizontal" method="POST" id="formUpdateInformasi" action="javascript:void(0)">
              <div class="form-group">
                ...
                <div class="col-sm-10">
                  <input type="text" class="form-control" id="inputName" name="nama_lengkap_siswa" value="<?= isset($user) ? $user->nama_lengkap_siswa : '' ?>">
                ...
                <div class="col-sm-10">
                  <select class="form-control"  name="jenis_kelamin_id">
                    ...
                    <?php foreach ($jenis_kelamin as $jenis_kelamin): ?>
                      <option <?= $user->id_jenis_kelamin == $jenis_kelamin->id_jenis_kelamin ? 'selected' : '' ?> value="<?= $jenis_kelamin->id_jenis_kelamin ?>"><?= $jenis_kelamin->jenis_kelamin ?></option>
                    <?php endforeach ?>
                  </select>
                  <span class="help-block"></span>
                </div>
              ...
              <div class="form-group">
                <div class="col-sm-offset-2 col-sm-10">
                  ...
                  <a href="<?= site_url()?>siswa" class="btn btn-flat btn-danger btn-danger">Batal</a>
                </div>
              </div>
            </form>
          </div>
...
<script>
  const URL_UPDATE_PASSWORD = `<?= site_url('siswa/profile/password/update/')?>`;
  const URL_UPDATE_INFORMASI = `<?= site_url('siswa/profile/informasi/update/')?>`;
  ...
  function saveInformasiUserEdit() {
    ...
    let id_siswa = `<?= $this->session->userdata('siswa_id') ?>`;

    var formData = new FormData($('#formUpdateInformasi')[0]);
    $.ajax({
      url : `${URL_UPDATE_INFORMASI}${id_siswa}`,
      type: "POST",
      data: formData,
      contentType: false,
      processData: false,
