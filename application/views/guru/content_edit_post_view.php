				...
				<form action="javascript:void(0)" method="POST" id="formEditPost">
					<div class="box-body">
						...
						<div class="box-footer clearfix">
							...
							<button type="botton" id="btnSave" onclick="save()" class="pull-right btn btn-flat btn-primary">Edit Kiriman</button>

							<button class="btn btn-flat btn-danger pull-right" onclick="window.history.go(-1)" type="reset" id="cancel">Kembali</button>
...
<script>
	const URL_EDIT_POST = `<?= site_url()?>guru/post/edit/store/`;
		...
		$.ajax({
			...
			success: function(data){
				if(data.status){
					...
					.then((success) => {
						if (success) {
							location.href = `<?= site_url()?>guru/post/${ID_POST}`;
						}
					}).then(() => {
						setTimeout(() => {
							location.href = `<?= site_url()?>guru/post/${ID_POST}`;
